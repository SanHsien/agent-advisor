...
Use these contracts with Agent Advisor for Codex's namespaced, role-pinned native custom agents.
...
file cannot satisfy the check, and the standing attestation never satisfies auxiliary
preflight. Then preflight only auxiliaries selected by the route:
none for solo; Luna / Max or Terra / High for delegate; fresh Sol / High for audit;
and one selected implementer plus fresh Sol reviewer for full. Cache each successful
check only for the task. After spawning, complete the selected role's routing and
reviewer-isolation checks before accepting the result:
...
~~~text
ROLE
Act as Agent Advisor for Codex's default routine implementation worker. Execute the supplied
...
~~~text
ROLE
Act as Agent Advisor for Codex's explicit high-complexity escalation worker. Resolve the supplied
specification within the settled architecture, preserve every stated interface and
constraint, and surface ambiguity instead of redesigning the architecture.
